...
| 功能 | 默认 | 安装命令 | 什么时候问 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.24` | 永远需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建议开 | `python -m pip install "vault-for-llm[mcp]==0.6.24"` | runtime 可以接 local stdio MCP tools。 |
...
features、既有 Obsidian vault 路径、是否做第一次导入，以及是否生成 cron、LaunchAgent 或 n8n 自动同步模板。
