...
def _write_brief_markdown(project: Path, payload: dict[str, Any], *, brief_path: str | Path) -> str:
    ...
    lines = [
        ...
        f"- open sync conflicts: `{int(summary.get('open_sync_conflicts') or 0)}`",
...
def _write_fleet_health_markdown(project: Path, payload: dict[str, Any], *, health_path: str | Path) -> str:
    ...
    lines = [
        ...
        f"- sync status: `{_md_text(summary.get('sync_status', 'idle'))}`",
        f"- open sync conflicts: `{int(summary.get('open_sync_conflicts') or 0)}`",
        f"- sync revisions / audit events: `{int(summary.get('sync_revisions') or 0)}` / `{int(summary.get('sync_audit_events') or 0)}`",
        "",
        "## Cards",
        "",
    ]
