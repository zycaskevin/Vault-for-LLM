...
| 系統 | 使用方式 |
|---|---|
| Hermes Agent | 設定 `vault-mcp`，讓 Agent 使用 search/read/propose tools；用 CLI 跑 dream report、backup、onboarding benchmark。 |
...
| 功能 | 預設 | 安裝指令 | 什麼時候問 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.40` | 永遠需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建議開 | `python -m pip install "vault-for-llm[mcp]==0.6.40"` | runtime 可以接 local stdio MCP tools。 |
...
長期 Agent 安裝也建議加 `--write-stable-venv-script`，或用
`--stable-venv ~/.hermes/venvs/vault-for-llm` 指定穩定 venv 路徑，讓精靈在
`agent-install/` 產生可檢查、可執行的穩定環境 bootstrap。
...
目前 source tree 已包含 v0.6.40 的 agent integration、OpenClaw adapter、Obsidian 匯入同步、benchmark proof 與品質 gate，並保留候選制記憶 workflow 與搜尋增強。白話說，Vault 現在不像一個誰都能亂塞紙條的抽屜，比較像一間有櫃台的小圖書館：
...
Vault-for-LLM `0.6.40` 已發布到 PyPI。

如果要讓 Agent 代為安裝，可以直接把這段交給 Hermes Agent、Codex、OpenCode、Claude Code、OpenClaw，或其他能執行本機指令的 Agent：

```text
幫這個專案安裝 Vault-for-LLM。使用 PyPI 套件 vault-for-llm[mcp]==0.6.40。
先詢問 vault database 要 shared、private、domain-specific 還是 temporary。
詢問穩定的 project directory，長期使用的 vault 不要放在 /tmp。
如果要建立或搬移 Python virtualenv，優先使用 ~/.hermes/venvs/vault-for-llm/ 這類穩定路徑，不要放在 /tmp。
...
| `vault remote search "query" --agent-id remote-agent --json` | 搜尋 Supabase read-only 遠端記憶視圖 |
...
若選 Supabase，精靈也可以產生 shell、n8n、Coze remote reader 範本，讓 hosted agent 使用 `vault remote smoke/search/map/read` 或 Supabase RPC 讀安全摘要。
`headroom` 是進階可選的 context 壓縮功能，不是 Vault 記憶治理的必要條件；只有在長 logs、大量 tool output 或 token 壓力明確時才建議開啟。
如果目前 Python 環境在 `/tmp`，精靈會建議產生 `agent-install/setup-stable-venv.sh`；非互動安裝可用 `--write-stable-venv-script` 或 `--stable-venv PATH` 直接要求產生。
...
# 套用 docs/supabase_read_policy.sql 後，hosted reader 可用 CLI 遠端讀取
...
# 或讓 setup-agent 產生每日同步與 shell/n8n/Coze remote-reader 範本
vault setup-agent \
  --non-interactive \
  --agent profile-agent \
  --scope shared \
...
| Probe | 結果 | 注意事項 |
|---|---:|---|
| Repo onboarding fixture | Vault top-k/source/read-range guidance `28/28`；Codex transcript baseline `7/28`；Hermes profile transcript baseline `3/28` | 28 題 source-aware project benchmark；private transcripts 不提交進 repo |
