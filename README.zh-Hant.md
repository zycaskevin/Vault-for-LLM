...
Obsidian 匯入，再問我要不要開後續自動同步，最後跑 search/read/propose smoke test。
```
...
| 功能 | 預設 | 安裝指令 | 什麼時候問 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.24` | 永遠需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建議開 | `python -m pip install "vault-for-llm[mcp]==0.6.24"` | runtime 可以接 local stdio MCP tools。 |
...
不要偷偷開 semantic 或 Supabase extras；它們會增加較重的依賴、模型/provider 設定，或遠端憑證。
