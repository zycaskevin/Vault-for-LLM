...
Scope: public README feature/capability claims after the v0.6.67 MCP session-capture update. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.67` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
| C09 | Agent session transcripts can be captured into reviewable memory candidates. | usable-alpha | `vault capture session` and MCP `vault_capture_session` parse JSONL, Markdown, and text transcripts for reusable decisions, pitfalls, workflows, and source-of-truth lines. They default to dry-run preview and write only `memory_candidates` when `--write-candidates` / `write_candidates=true` is set. MCP capture is available in review/maintenance profiles, not core, and requires explicit permission for absolute transcript paths. | `tests/test_session_capture.py` covers preview, writes, nested JSONL, privacy rejection, and CLI dispatch. `tests/test_mcp_memory.py` covers MCP preview, explicit writes, profile visibility, and path safety. |
| C10 | Policy-based automation is report-first by default and candidate-only when it writes. | usable-alpha | `vault automation plan/run/cycle/report/inbox/eval/doctor`; reports include a dry-run diff and action ledger. `automation inbox` provides a compact read-only review queue that hides candidate content by default and prioritizes privacy-blocked, sensitive, duplicate, weak-quality, and automation-generated candidates. `vault automation inbox --write-handoff` writes `reports/automation/inbox-latest.json`, and generated cron, LaunchAgent, and n8n schedules run it after the selected `cycle` or `run` command. MCP `vault_automation_inbox` exposes the same compact queue to review/maintenance agents without adding it to the core profile. Balanced policy can pre-fill Dream and Forgetting suggestions as memory candidates only when `--apply` is used, while `automation eval` and `automation cycle` turn promoted/rejected/blocked candidate feedback into bounded curation hints. `vault candidate-review` and MCP `vault_memory_review` let reviewers record rejected/blocked feedback without promotion. Dream can write duplicate-group `consolidation_suggestion` candidates for reviewed merge/archive decisions. Automation never promotes candidates or hard-deletes rows. Generated schedule templates default to `vault automation cycle`, can opt down to `run`, and omit `--apply` unless explicitly requested. | Automation tests, Agent setup tests, Dream/MCP tests, PyPI setup-agent smoke, and full pytest passed before release. |
...
python scripts/check_release_parity.py --tag v0.6.67
...
For release v0.6.67, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.67` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.67`.
