...
```
L0 Identity      → Who the agent is (injected every conversation)
...
# Add knowledge
guardrails add "My First Entry" --content "Something I learned today"

# Compile (raw → database + compiled)
guardrails compile

# Search
guardrails search "my query"
...
```
your-project/
├── guardrails.yaml          ← Project config (auto-generated by `guardrails init`)
├── L0-identity/             ← Identity (injected every conversation)
│   └── identity.md
...
2. Read `L0-identity/identity.md` to understand the user
3. Read `L1-core-facts/current-projects.md` for current state
4. Use `guardrails search "query"` for semantic search
...
| Command | Description |
|---------|-------------|
| `guardrails init` | Initialize a new project |
| `guardrails doctor` | Health check |
| `guardrails add "Title" --content "..."` | Add knowledge entry |
| `guardrails add "Title" --file notes.md` | Add from file |
| `guardrails compile` | Compile raw/ → database + compiled/ |
| `guardrails search "query"` | Search (auto: keyword + semantic) |
| `guardrails list` | List all entries |
| `guardrails stats` | Show database statistics |
| `guardrails lint` | Run quality checks |
| `guardrails graph build` | Build knowledge graph |
| `guardrails graph show` | Show graph summary |
