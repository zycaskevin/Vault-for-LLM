...
Scope: public README feature/capability claims after the v0.6.97 learned automation inbox ranking update. Localized README files should mirror the same maturity and non-goal language.
...
python scripts/check_release_parity.py --tag v0.6.97
...
For release v0.6.97, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.97` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.97`.
...
- `vault setup-agent` writes `agent-install/README-runtime-update-playbook.md` and `runtime-update-playbook.json`.
