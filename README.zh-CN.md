...
├── guardrails.db             # vault 生成的本地 SQLite database
└── templates/                # 起始模板
```

### 历史命名说明

部分内部模块与文件名仍沿用历史名称 `guardrails_lite` / `guardrails.db`。公开产品名称是 **Vault-for-LLM**，公开命令是 `vault` 与 `vault-mcp`。这些旧命名会在 alpha 阶段为了兼容性暂时保留。
...
- 公开 CLI 是 `vault`，但部分内部名称仍含 `guardrails`。
