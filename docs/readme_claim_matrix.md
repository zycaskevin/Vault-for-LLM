...
Scope: public README feature/capability claims after the v0.6.98 explainable memory importance update. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.98` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
python scripts/check_release_parity.py --tag v0.6.98
...
For release v0.6.98, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.98` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.98`.
