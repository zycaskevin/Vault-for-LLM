    ...
    *,
    ...
    runtime_playbook_readme_path.write_text(
        "\n".join(
            [
                ...
                "4. If the notice is stale, missing Agents, or upgrade state is unclear, run MCP doctor: `vault_update_status` with `doctor=true` and this runtime's `agent_id`.",
                f"5. Read the compact handoff: `{shell_join(handoff_command)}`.",
                ...
                "6. Search/read only when the task needs deeper context.",
                "",
    ...
    def adapter_markdown(title: str, *, target: str, note: str, openclaw: bool = False) -> str:
        lines = [
            ...
            f"4. Read the compact automation handoff: `{shell_join(handoff_command)}`.",
            "5. If handoff includes `fleet_health_content`, read that shared health preface before the main handoff.",
            "6. If handoff includes `pipeline_receipt_content`, read the memory-ingestion receipt before review cards.",
            "7. If handoff includes `review_summary_content`, read those 5% review cards before deeper reports.",
            "8. If handoff includes `learning_health_content`, use it as the learning loop status.",
            "9. Run MCP doctor when status freshness or runtime updates are unclear.",
            "10. Search only when the task needs more context.",
            "11. Use bounded reads before citing memory.",
            "12. Propose durable lessons as candidates; do not auto-promote them.",
            "",
            "MCP server:",
            "",
            "```bash",
    ...
    readme_path.write_text(
        "\n".join(
            [
                ...
                f"4. Read `{shell_join(handoff_command)}` for the latest compact memory automation handoff.",
                "5. If the handoff includes fleet health, read that shared health preface before the individual handoff.",
                "6. If the handoff includes review-summary cards, read them before deeper reports.",
                "7. If the handoff includes learning-health, use it as the learning loop status.",
                "8. Use MCP doctor when checking whether every local runtime has the fresh shared notice.",
                "9. Search only when the task or handoff needs more detail.",
                "10. Read bounded evidence before citing memory.",
                "11. Propose new durable memory as candidates.",
                "",
                "Safety boundary:",
                "",
                "- no auto-upgrade",
