...
```text
Install Vault-for-LLM for this project with vault-for-llm[mcp]==0.6.72.
...
```bash
python -m pip install "vault-for-llm[mcp]==0.6.72"
vault setup-agent
```
...
`reports/automation/cycle-latest.json`, a compact daily memory workbench for the
next agent.
