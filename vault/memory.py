...
def _gate_payload(privacy: dict, duplicate: dict, metadata: dict) -> dict:
    return {"privacy": privacy, "duplicate": duplicate, "metadata": metadata}
...
def create_candidate(db: VaultDB, **kwargs) -> dict:
    ...
    gates = _gate_payload(privacy, duplicate, metadata)
    ...
    result = {
        ...
        "knowledge_id": None,
        "gates": {"privacy": privacy["status"], "duplicate": duplicate["status"], "metadata": metadata["status"]},
        "gate_payload": gates,
    }
    if not rejected:
...
def propose_memory(db: VaultDB, mode: str = "candidate", **kwargs) -> dict:
    ...
    if mode == "promote_if_safe" and result["status"] == "candidate_created" and result["gates"]["privacy"] != "fail":
        promoted = promote_candidate(db, result["candidate_id"], confirm=True)
...
def promote_candidate(db: VaultDB, candidate_id: str, *, confirm: bool = False, project_dir: str | Path | None = None, compile: bool = True, build_map: bool = True) -> dict:
    ...
    if privacy["status"] == "fail" or metadata["status"] == "fail":
        db.update_memory_candidate(candidate_id, status="rejected", privacy_status=privacy["status"], duplicate_status=duplicate["status"], gate_payload_json=json.dumps(gates, ensure_ascii=False, sort_keys=True))
    ...
    if compile:
        ...
        row = db.conn.execute("SELECT id FROM knowledge WHERE source LIKE ? ORDER BY id DESC LIMIT 1", (f"%{raw_path.name}%",)).fetchone()
    ...
    if knowledge_id is None:
        knowledge_id = db.add_knowledge(
            ...
            source=str(raw_path.relative_to(root / "raw")),
        )
        if build_map:
            VaultCompiler(root, db=db, embed_provider=None)._refresh_document_map(knowledge_id)

    db.update_memory_candidate(candidate_id, status="promoted", privacy_status=privacy["status"], duplicate_status=duplicate["status"], gate_payload_json=json.dumps(gates, ensure_ascii=False, sort_keys=True), promoted_knowledge_id=knowledge_id)
    return {
        "status": "promoted",
        "candidate_id": candidate_id,
