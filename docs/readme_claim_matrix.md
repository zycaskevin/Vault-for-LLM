...
Scope: public README feature/capability claims after the v0.6.66 scheduled inbox handoff update. Localized README files should mirror the same maturity and non-goal language.
...
| C04 | Manual quickstart works from PyPI. | stable | `vault-for-llm[mcp]==0.6.66` installs and exposes `vault`. | Clean Python 3.11 PyPI install smoke is part of release closeout. |
...
| C09 | Agent session transcripts can be captured into reviewable memory candidates. | usable-alpha | `vault capture session` parses JSONL, Markdown, and text transcripts for reusable decisions, pitfalls, workflows, and source-of-truth lines. It defaults to dry-run preview and writes only `memory_candidates` when `--write-candidates` is set. | `tests/test_session_capture.py` covers preview, writes, nested JSONL, privacy rejection, and CLI dispatch. |
...
python scripts/check_release_parity.py --tag v0.6.66
...
For release v0.6.66, clean Python 3.11 PyPI install closeout should verify:

- `vault-for-llm[mcp]==0.6.66` installs from PyPI.
- `vault --version` returns `vault-for-llm 0.6.66`.
