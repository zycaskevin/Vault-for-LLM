...
| 功能 | 默认 | 安装命令 | 什么时候问 |
|---|---|---|---|
| `core` | 是 | `python -m pip install vault-for-llm==0.6.25` | 永远需要：本地 Markdown、SQLite、keyword search。 |
| `mcp` | MCP-capable agent 建议开 | `python -m pip install "vault-for-llm[mcp]==0.6.25"` | runtime 可以接 local stdio MCP tools。 |
...
`/tmp/...` 是一次性测试工作目录，不是包真正安装的位置，也不适合拿来当长期共享记忆库。正式共用时，请选稳定路径，例如 `~/Vaults/my-project`，并让每个可信 Agent 都指向同一个 `project-dir`。
...
当前 source tree 已包含 v0.6.25 的 agent integration、OpenClaw adapter、Obsidian 导入同步、benchmark proof 与质量 gate，并保留候选制记忆 workflow 与搜索增强。白话说，Vault 现在不像一个谁都能乱塞纸条的抽屉，更像一间有前台的小图书馆：
...
features（MCP、semantic、Supabase、Headroom、dev）、是否立即安装已选功能的 dependencies、既有 Obsidian vault 路径、是否做第一次导入，以及是否生成 cron、LaunchAgent 或 n8n 自动同步模板。若选 semantic 并确认安装依赖，精灵也可以下载并设置本地 ONNX embedding model。
